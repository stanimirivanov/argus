...
func TestCanonicalSnapshotIsOrderIndependentAndDoesNotMutateInput(t *testing.T) {
	...
	original := cloneSnapshotForTest(right)

	canonicalLeft := canonicalSnapshot(left)
	canonicalRight := canonicalSnapshot(right)
	...
	leftFingerprint, err := snapshotFingerprint(canonicalLeft)
	if err != nil {
		t.Fatalf("fingerprint left snapshot: %v", err)
	}
	rightFingerprint, err := snapshotFingerprint(canonicalRight)
	if err != nil {
		t.Fatalf("fingerprint right snapshot: %v", err)
	}
...
func cloneSnapshotForTest(snapshot catalog.Snapshot) catalog.Snapshot {
	clone := snapshot
